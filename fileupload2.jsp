...
PreparedStatement ps1=con1.prepareStatement("insert into fileupload1(uid,fname,fid,size,date,ff,owner,fileid,status) values(?,?,?,?,?,?,'"+ccc+"',?,'"+status+"')");
		//File f = new File(saveFile);
				ps1.setString(1,un);
                ps1.setString(3,fn1);
                ps1.setString(2,fid1);
fis = new FileInputStream(ff);


	  
	  
		 double size1=ff.length();
		  double  size11=size1/1024;
		  double size21=Math.round(size1*100.0)/100.0;
		  String size31=Double.toString(size21);
		  session.setAttribute("filesize",size31);

	
        ps1.setBinaryStream(7, (InputStream)fis, (int)(ff.length()));
        ps1.setString(4,size31);
